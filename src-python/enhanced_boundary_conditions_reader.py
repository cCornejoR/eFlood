#!/usr/bin/env python3
"""
🌊 Enhanced Boundary Conditions Reader for HEC-RAS HDF5 Files
Extracts specific boundary condition names like "ENTRADA_RIO_CHICO" from HDF5 files
"""
...
class EnhancedBoundaryConditionsReader:
    """
    🔍 Enhanced reader for HEC-RAS boundary conditions from HDF5 files
    """
    
    def __init__(self, hdf_file_path: str):
        """
        Initialize the enhanced boundary conditions reader
    ...
    def extract_boundary_conditions(self) -> Dict[str, Any]:
        """
        Extract all boundary conditions with specific names from the HDF5 file
    ...
    def _search_specific_boundary_conditions(self, hf: h5py.File) -> List[Dict[str, Any]]:
        """
        Search for specific boundary conditions with actual names
        ...
        # Order matters - more specific paths first to avoid duplicates
        priority_paths = [
            ...
            # Only search the general path if specific ones don't exist
        ...
        for bc_path in priority_paths:
            if bc_path in hf:
                logger.info(f"Searching for specific BCs at: {bc_path}")
                bc_group = hf[bc_path]
                path_found_bcs = False

                # Extract each boundary condition
                ...
                # If we found BCs in a specific path, mark it
                if path_found_bcs:
                    found_in_specific_paths = True

                # If this is the general path and we already found BCs in specific paths, skip it
                if bc_path == "Event Conditions/Unsteady/Boundary Conditions" and found_in_specific_paths:
                    logger.info("Skipping general BC path since we found BCs in specific paths")
                    continue
    ...
    def _create_bc_info(self, bc_name: str, bc_path: str, bc_item) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(bc_item, 'keys'):
                ...
                # Try to get time steps from flow data
                for data_key in bc_item.keys():
                    try:
                        ...
                        continue
            elif hasattr(bc_item, 'shape'):
                # This is a dataset itself
                bc_info["data_available"] = True
                bc_info["time_steps"] = int(bc_item.shape[0]) if len(bc_item.shape) >= 1 else 0
    ...
    def _recursive_search_for_bcs(self, group, path="") -> List[Dict[str, Any]]:
        """
        Recursively search for boundary conditions
    ...
    def _looks_like_boundary_condition(self, name: str, path: str) -> bool:
        ...
        # Look for specific patterns that indicate boundary conditions
        bc_indicators = [
            'entrada', 'salida', 'inlet', 'outlet', 'inflow', 'outflow',
            'rio', 'river', 'canal', 'channel', 'bc_line', 'boundary_line',
            'flow_hydrograph', 'stage_hydrograph'
        ]
        
        # Check if name contains BC indicators
        for indicator in bc_indicators:
            if indicator in name_lower:
                return True
        
        # Check if path suggests it's a boundary condition
        if 'boundary' in path_lower and 'condition' in path_lower:
            return True
    ...
    def _remove_duplicates(self, boundary_conditions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for bc in boundary_conditions:
            ...
            identifier = f"{bc['name']}_{bc['type']}"

            if identifier not in seen:
                seen.add(identifier)
                unique_bcs.append(bc)
                logger.info(f"Keeping unique BC: {bc['name']} ({bc['type']})")
            else:
                logger.info(f"Removing duplicate BC: {bc['name']} ({bc['type']})")
    ...
    def _generate_description(self, bc_name: str) -> str:
        ...
        if 'entrada' in name_lower or 'inlet' in name_lower:
            if 'rio' in name_lower or 'river' in name_lower:
                return f"Condición de entrada de caudal en {bc_name}"
            else:
                return f"Hidrograma de entrada: {bc_name}"
        elif 'salida' in name_lower or 'outlet' in name_lower:
            return f"Condición de salida: {bc_name}"
        else:
    ...
    def _extract_time_series_from_group(self, group, time_series: Dict, base_path: str):
        """
        Recursively extract time series data from a group
        
        Args:
            group: HDF5 group
            time_series: Dictionary to store time series data
        ...
        for key in group.keys():
            ...
            if hasattr(item, 'shape') and len(item.shape) >= 1:
                try:
                    ...
                    if np.any(np.isfinite(data)):
                        ...
                        time_series[current_path] = {
                            "data": clean_data.tolist()[:100],  # Limit to first 100 points
                ...
                self._extract_time_series_from_group(item, time_series, current_path)
...
def main():
    ...
    if len(sys.argv) < 2:
        print(json.dumps({
            "success": False,
            "error": "Usage: python enhanced_boundary_conditions_reader.py <hdf_file_path>"
        }))
        sys.exit(1)
    ...
    try:
        reader = EnhancedBoundaryConditionsReader(hdf_file_path)
