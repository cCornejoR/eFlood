...
class HydraulicPlotter:
    ...
    def create_water_surface_plot(self, data: np.ndarray, time_hours: Optional[np.ndarray] = None,
                                 title: str = "Water Surface Elevation") -> str:
        """
        ...
        try:
            ...
            ax.fill_between(time_hours, data, alpha=0.4, color=self.water_colors[0],
                           label='Water Surface')
            ...
            ax.axhline(y=max_wse, color='red', linestyle='--', alpha=0.7,
                      label=f'Máximo: {max_wse:.2f} m')
            ax.axhline(y=mean_wse, color='orange', linestyle='--', alpha=0.7,
                      label=f'Promedio: {mean_wse:.2f} m')
            ax.axhline(y=min_wse, color='green', linestyle='--', alpha=0.7,
    ...
    def create_velocity_plot(self, data: np.ndarray, time_hours: Optional[np.ndarray] = None,
                           title: str = "Velocity") -> str:
        """
        ...
        try:
            ...
            # Color zones
            ax.axhspan(0, critical_velocity, alpha=0.1, color='green',
                      label='Flujo Subcrítico')
            ax.axhspan(critical_velocity, high_velocity, alpha=0.1, color='yellow',
                      label='Flujo Crítico')
            ax.axhspan(high_velocity, np.max(data) * 1.1, alpha=0.1, color='red',
    ...
    def create_depth_plot(self, data: np.ndarray, time_hours: Optional[np.ndarray] = None,
                         title: str = "Water Depth") -> str:
        """
        ...
        try:
            ...
            ax.axhspan(0, shallow, alpha=0.1, color='yellow', label='Agua Poco Profunda')
            ax.axhspan(shallow, moderate, alpha=0.1, color='blue', label='Profundidad Moderada')
            ax.axhspan(moderate, np.max(data) * 1.1, alpha=0.1, color='darkblue',
                      label='Agua Profunda')
    ...
    def create_hydrograph_advanced(self, discharge_data: np.ndarray,
                                  time_hours: Optional[np.ndarray] = None,
                                  stage_data: Optional[np.ndarray] = None,
                                  title: str = "Hidrograma Avanzado") -> str:
        """
        ...
        try:
            ...
            line1 = ax1.plot(time_hours, discharge_data, color=color1, linewidth=2,
                           label='Caudal')[0]
            ...
            if stage_data is not None:
                ...
                line2 = ax2.plot(time_hours, stage_data, color=color2, linewidth=2,
                               linestyle='--', label='Nivel')[0]
            ...
            ax1.annotate(f'Pico: {peak_flow:.1f} m³/s\nt = {peak_time:.1f} h',
                        xy=(peak_time, peak_flow), xytext=(peak_time + len(time_hours)*0.1, peak_flow),
    ...
    def create_flow_duration_curve(self, data: np.ndarray,
                                  title: str = "Curva de Duración de Caudales") -> str:
        """
        ...
        try:
            ...
            ax.semilogy(exceedance, sorted_data, linewidth=2, color=self.water_colors[0])
            ax.fill_between(exceedance, sorted_data, alpha=0.3, color=self.water_colors[1])
            ...
            for p in percentiles:
                ...
                if idx < n:
                    ...
                    ax.text(p, flow_value * 1.1, f'Q{p}: {flow_value:.1f}',
...
def main():
    ...
    try:
        ...
        if command == "water_surface":
            ...
            stage_data = np.array(data_dict.get('stage_data', [])) if 'stage_data' in data_dict else None
            image_base64 = plotter.create_hydrograph_advanced(data, time_hours, stage_data)
