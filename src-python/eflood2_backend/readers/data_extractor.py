...
class DataExtractor:
            ...
            if data.ndim == 1:
                ...
                for i, value in enumerate(data):
                    if not (np.isnan(value) or np.isinf(value)):  # Skip invalid values
                        cleaned_data.append({"index": i, "column_2": float(value)})
            elif data.ndim == 2:
                # 2D data - use first column as index, second as column_2
                for i, row in enumerate(data):
                    if len(row) >= 2:
                        index_val = (
                            row[0] if not (np.isnan(row[0]) or np.isinf(row[0])) else i
                        )
                        col2_val = (
                            row[1] if not (np.isnan(row[1]) or np.isinf(row[1])) else 0
                        )
                        cleaned_data.append(
                            {"index": float(index_val), "column_2": float(col2_val)}
                        )
                    elif len(row) >= 1:
                        # Only one column available, use row index as index
                        col2_val = (
                            row[0] if not (np.isnan(row[0]) or np.isinf(row[0])) else 0
                        )
                        cleaned_data.append({"index": i, "column_2": float(col2_val)})
    ...
    def extract_dataset_data(
        self, dataset_path: str, max_rows: int = 1000, clean_for_hydrograph: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def create_time_series_plot(
        self, dataset_path: str, title: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            with h5py.File(self.file_path, "r") as f:
                ...
                if data.ndim == 1:
                    ...
                    if data.shape[1] <= 10:  # Plot multiple series if not too many
                        ...
                        ax.text(
                            0.02,
                            0.98,
                            f"Showing 5 of {data.shape[1]} series",
                ...
                # Set title
                plot_title = (
                    title or f'Data Visualization: {dataset_path.split("/")[-1]}'
                )
    ...
    def create_hydrograph(
        self, dataset_path: str, time_column: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            data_info = self.extract_dataset_data(
                dataset_path, max_rows=10000, clean_for_hydrograph=True
            )
            ...
            ax.plot(
                x_values, y_values, linewidth=2, color="#1d4ed8", label="Hidrograma"
            )
            ...
            ax.set_title(
                f'Hidrograma: {dataset_path.split("/")[-1]}',
                fontsize=16,
                fontweight="bold",
            )
    ...
    def create_export_file(
        self, dataset_path: str, format: str, output_dir: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            safe_name = (
                dataset_path.replace("/", "_").replace("\\", "_").replace(" ", "_")
            )
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python hdf_data_extractor.py <hdf_file> <command> [dataset_path] [options]"
        )
        print("Commands:")
