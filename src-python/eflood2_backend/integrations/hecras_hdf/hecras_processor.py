        ...
        if hasattr(ras_data, "ManningNZones") and ras_data.ManningNZones:
            ...
            for zone_id, zone_data in manning_zones.items():
                if len(zone_data) >= 2:
                    ...
                    if (
                        manning_value != -9999.0
                        and np.isfinite(manning_value)
                        and manning_value > 0
                    ):
            ...
            manning_data["formatted_table"] = tabulate(
                table_rows, headers=headers, tablefmt="grid", stralign="left"
            )
    ...
    try:
        ...
        with h5py.File(hdf_file, "r") as hf:
            ...
            if "File Version" in hf.attrs:
                file_version = (
                    hf.attrs["File Version"].decode()
                    if isinstance(hf.attrs["File Version"], bytes)
                    else str(hf.attrs["File Version"])
                )
                ...
                if "Geometry" in hf and "2D Flow Areas" in hf["Geometry"]:
                    ...
                    if flow_areas:
                        ...
                        if "Faces Center Coordinate" in hf[area_path]:
                            metadata["num_faces"] = len(
                                hf[f"{area_path}/Faces Center Coordinate"]
                            )
                else:
                    metadata["flow_areas"] = []
        ...
        return {"success": False, "error": f"Error reading HDF file: {str(e)}"}
...
def process_hec_ras_data(hdf_file, terrain_file=None):
    ...
    try:
        ...
        metadata = {
            ...
            "flow_areas": (
                [str(area) for area in ras_data.TwoDAreaNames]
                if hasattr(ras_data, "TwoDAreaNames")
                else []
            ),
            "cell_counts": (
                [int(count) for count in ras_data.TwoDAreaCellCounts]
                if hasattr(ras_data, "TwoDAreaCellCounts")
                else []
            ),
            "solution_times": (
                len(ras_data.solution_time) if hasattr(ras_data, "solution_time") else 0
            ),
            "boundaries": {
                "total": (
                    int(ras_data.totalBoundaries)
                    if hasattr(ras_data, "totalBoundaries")
                    else 0
                ),
                "names": (
                    [str(name) for name in ras_data.boundaryNameList]
                    if hasattr(ras_data, "boundaryNameList")
                    else []
                ),
                "types": (
                    [str(type_) for type_ in ras_data.boundaryTypeList]
                    if hasattr(ras_data, "boundaryTypeList")
                    else []
                ),
            },
            "processor": "HECRAS-HDF Integrated (pyHMT2D compatible)",
            "terrain_provided": (
                terrain_file is not None and os.path.exists(terrain_file)
                if terrain_file
                else False
            ),
            "datasets_available": {
                "depth_data": (
                    len(ras_data.TwoDAreaCellDepth) > 0
                    if hasattr(ras_data, "TwoDAreaCellDepth")
                    else False
                ),
                "wse_data": (
                    len(ras_data.TwoDAreaCellWSE) > 0
                    if hasattr(ras_data, "TwoDAreaCellWSE")
                    else False
                ),
                "velocity_data": (
                    len(ras_data.TwoDAreaPointVx) > 0
                    if hasattr(ras_data, "TwoDAreaPointVx")
                    else False
                ),
            },
        }
        ...
        return {"success": True, "metadata": metadata, "ras_data_object": "initialized"}

    except Exception as e:
        logger.error(f"Error processing HEC-RAS data: {str(e)}")
        return {"success": False, "error": f"Error processing HEC-RAS data: {str(e)}"}
...
def create_hydrograph(hdf_file, cell_id=0, terrain_file=None):
    ...
    try:
                        ...
                        for bc_name in bc_group.keys():
                            ...
                            if hasattr(bc_item, "keys"):  # It's a group
                                for data_key in bc_item.keys():
                                    if (
                                        "flow" in data_key.lower()
                                        or "stage" in data_key.lower()
                                    ):
                                        try:
                                            ...
                                            if len(data) > 0:
                                                ...
                                                ax.plot(
                                                    ...
                                                    data,
                                                    linewidth=2,
                                                    label=f"{bc_name} - {data_key}",
                                                )
                                                boundary_data_found = True
                                                logger.info(
                                                    f"Plotted data from {bc_name}/{data_key}"
                                                )
                                        except Exception as e:
                                            logger.warning(
                                                f"Error reading {bc_name}/{data_key}: {e}"
                                            )
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    if len(data) > 1 and not boundary_data_found:
                                        times = range(len(data))
                                        ax.plot(
                                            times, data, linewidth=2, label=current_path
                                        )
                                        boundary_data_found = True
                                        logger.info(
                                            f"Plotted time series from {current_path}"
                                        )
                                        return
                                except:
                                    pass
        ...
        if not boundary_data_found:
            if (
                hasattr(ras_data, "TwoDAreaCellDepth")
                and len(ras_data.TwoDAreaCellDepth) > 0
            ):
        ...
        if boundary_data_found:
            ...
            ax.text(
                0.5,
                0.5,
                "No se encontraron datos de condiciones de contorno\no datos de series temporales disponibles",
        ...
        return {
            ...
            "note": (
                "Hidrograma generado desde condiciones de contorno"
                if boundary_data_found
                else "No se encontraron datos de condiciones de contorno"
            ),
        }

    except Exception as e:
        logger.error(f"Error creating hydrograph: {str(e)}")
        return {"success": False, "error": f"Error creating hydrograph: {str(e)}"}
...
def create_depth_map(hdf_file, terrain_file=None):
    ...
    try:
        ...
        return {"success": False, "error": f"Error creating depth map: {str(e)}"}
...
def create_profile(hdf_file, terrain_file=None):
    ...
    try:
        ...
        return {"success": False, "error": f"Error creating profile: {str(e)}"}
...
def get_vtk_export_info(hdf_file, terrain_file=None):
    ...
    try:
        ...
        export_info = {
            ...
            "metadata": {
                "file_version": (
                    ras_data.version if hasattr(ras_data, "version") else "Unknown"
                ),
                "units": ras_data.units if hasattr(ras_data, "units") else "Unknown",
                "flow_areas": (
                    [
                        area.decode("utf-8") if isinstance(area, bytes) else str(area)
                        for area in ras_data.TwoDAreaNames
                    ]
                    if hasattr(ras_data, "TwoDAreaNames")
                    else []
                ),
                "cell_counts": (
                    ras_data.TwoDAreaCellCounts
                    if hasattr(ras_data, "TwoDAreaCellCounts")
                    else []
                ),
                "solution_times": (
                    len(ras_data.solution_time)
                    if hasattr(ras_data, "solution_time")
                    else 0
                ),
                "total_timesteps": (
                    len(ras_data.solution_time)
                ...
                "start_time": (
                    ras_data.start_time if hasattr(ras_data, "start_time") else "N/A"
                ),
                "end_time": (
                    ras_data.end_time if hasattr(ras_data, "end_time") else "N/A"
                ),
                "terrain_provided": (
                    terrain_file is not None and os.path.exists(terrain_file)
                    if terrain_file
        ...
        return {"success": False, "error": f"Error getting VTK export info: {str(e)}"}
...
def export_to_vtk(
    ...
    try:
        ...
        if export_type == "max_values":
            ...
            result = export_max_values_vtk(
                ras_data, output_directory, base_name, progress_callback
            )
        else:
            # Export all time steps (default behavior)
            result = export_all_timesteps_vtk(
                ras_data, output_directory, base_name, progress_callback
            )
        ...
        return {"success": False, "error": f"Error exporting to VTK: {str(e)}"}


def export_all_timesteps_vtk(
    ras_data, output_directory, base_name, progress_callback=None
):
...
def export_max_values_vtk(
    ...
    try:
        ...
        if len(max_data) > 0:
            area_index = 0
            area_name = (
                str(ras_data.TwoDAreaNames[area_index])
                .replace("b'", "")
                .replace("'", "")
            )
            ...
            output_file = os.path.join(
                output_directory, f"{base_name}_{area_name}_MaxValues.vtk"
            )
    ...
    try:
        ...
        for area_index, area_name in enumerate(ras_data.TwoDAreaNames):
            if progress_callback:
                progress_callback(
                    f"Calculating maximum values for {area_name}",
                    area_index,
                    len(ras_data.TwoDAreaNames),
                )
            ...
            if (
                hasattr(ras_data, "TwoDAreaCellDepth")
                and len(ras_data.TwoDAreaCellDepth) > area_index
            ):
                ...
                if depth_data.size > 0:
                    ...
                    area_max_data["max_depth"] = np.zeros(
                        ras_data.TwoDAreaCellCounts[area_index]
                    )
            else:
                area_max_data["max_depth"] = np.zeros(
                    ras_data.TwoDAreaCellCounts[area_index]
            ...
            if (
                hasattr(ras_data, "TwoDAreaCellWSE")
                and len(ras_data.TwoDAreaCellWSE) > area_index
            ):
                wse_data = np.array(ras_data.TwoDAreaCellWSE[area_index])
                if wse_data.size > 0:
                    area_max_data["max_wse"] = np.max(wse_data, axis=0)
                else:
                    area_max_data["max_wse"] = np.zeros(
                        ras_data.TwoDAreaCellCounts[area_index]
                    )
            else:
                area_max_data["max_wse"] = np.zeros(
            ...
            if (
                hasattr(ras_data, "TwoDAreaPointVx")
                and len(ras_data.TwoDAreaPointVx) > area_index
                and hasattr(ras_data, "TwoDAreaPointVy")
                and len(ras_data.TwoDAreaPointVy) > area_index
            ):
                ...
                if vx_data.size > 0 and vy_data.size > 0:
                    ...
                    area_max_data["max_vx"] = vx_data[
                        np.argmax(velocity_magnitude, axis=0),
                        np.arange(vx_data.shape[1]),
                    ]
                    area_max_data["max_vy"] = vy_data[
                        np.argmax(velocity_magnitude, axis=0),
                        np.arange(vy_data.shape[1]),
                    ]
                else:
                    num_points = (
                        len(ras_data.TwoDAreaCellPoints[area_index])
                        if hasattr(ras_data, "TwoDAreaCellPoints")
                        else 1000
                    )
...
def extract_manning_table(hdf_file, terrain_file=None):
    ...
    try:
        ...
        return {"success": False, "error": f"Error extracting Manning table: {str(e)}"}
...
def main():
    ...
    if not os.path.exists(hdf_file):
        print(
            json.dumps({"success": False, "error": f"HDF file not found: {hdf_file}"})
        )
        sys.exit(1)
