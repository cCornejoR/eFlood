...
def _get_typelib_info(keyid, version):
    ...
    key = win32api.RegOpenKey(
        win32con.HKEY_CLASSES_ROOT, "TypeLib\\%s\\%s" % (keyid, version)
    )
    try:
        num = 0
        while 1:
...
def modify_HEC_RAS_file_with_key_two_lines(
    ras_file_name, key_name, value, value_second_line
):
    """
...
if __name__ == "__main__":
    ...
    modify_HEC_RAS_file_with_key_two_lines(
        ras_file_name="Muncie2D.u01",
        key_name="Stage Hydrograph",
        value=7,
