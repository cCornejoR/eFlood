...
class HEC_RAS_Plan(object):
    ...
    def __init__(
        self, title="", steady=False, geom_file="", flow_file="", plan_file=""
    ):
    ...
    def __repr__(self):
        return (
            ...
            'geometry file = "{}", '
            'flow file = "{}"'.format(
                self.title, self.steady, self.geom_file, self.flow_file
            )
        )
...
class HEC_RAS_Model(HydraulicModel):
    ...
    def __init__(self, version, faceless=False):
        ...
        if not (version in get_supported_hec_ras_versions()):
            print(
                "Specified HEC-RAS version",
                version,
                "is not supported in current version of pyHMT2D.",
            )
            print("Supported HEC-RAS versions are", get_supported_hec_ras_versions())
            print("Exitting.")
            sys.exit()

        self._installed_hec_ras_versions = (
            get_installed_hec_ras_versions()
        )  # get installed HEC-RAS versions on the
        # computer
        if (
            not self._installed_hec_ras_versions
        ):  # if the available HEC-RAS list is empty
            print(
                "No HEC-RAS installed on this computer. Install HEC-RAS first. Exitting."
            )
            sys.exit()
        else:
        ...
        if not (version in get_installed_hec_ras_versions()):
            print(
                "Specified HEC-RAS version",
                version,
                "is not installed on this computer. Install it first.",
                "Exitting.",
            )
            sys.exit()
    ...
    def init_model(self):
        ...
        if platform.system().startswith("Windows"):
            try:
                ...
                raise ImportError(
                    "Error in importing pywin32 package. Make sure it has been installed properly."
                )
        else:
            print("Current OS: ", platform.system())
            raise ImportError(
                "The use of pyHMT2D for HEC-RAS Model is only supported on Windows."
            )
        ...
        if self._RASController is not None:
            ...
            print(
                "HEC-RAS version ",
                self._RASController.HECRASVersion(),
        ...
        if self.getVersion() == "5.0.7":
            ...
            self._RASController = win32.gencache.EnsureDispatch(
                "RAS60.HECRASController"
            )
            ...
            raise Exception(
                "The specified version of HEC-RAS is not currently supported."
            )
        ...
        # print and confirm the HEC-RAS version
        print(
            "Successfully launched HEC-RAS version ",
            self._RASController.HECRASVersion(),
        )
        ...
        if PlanCount > 0:
            for i in range(PlanCount):
                ...
                if (not flow_file_steady) and (not flow_file_unsteady):
                    print(
                        "Error: no flow file specified in the current plan. Exitting."
                    )
                    sys.exit()
                ...
                plans.append(
                    HEC_RAS_Plan(PlanNames[i], steady, geom_file, flow_file, plan_file)
                )
    ...
    def set_current_plan(self, planName):
        ...
        if ret[0]:
            print(
                "Call to set_current_plan(...) is successful. Current plan is ",
                planName,
            )
        else:
            print(
                "Call to set_current_plan(...) failed. Current plan not changed. Check the validity of the plan name: ",
                planName,
            )
    ...
    def load_current_plan_results(self):
        ...
        if not os.path.isfile(currentPlanFile):
            raise Exception(
                "The result HDF file for current plan does not exist. Make sure to run HEC-RAS before calling this function."
            )
    ...
    def run_model(self):
        ...
        if self._RASController is None:
            print(
                "HEC-RAS model has not been initialized yet. Call init_model() first."
            )
            return

        if not (self._RASController.Project_Current()):
            print(
                "No HEC-RAS project has been opened yet. Call open_project(...) first."
            )
        ...
        if res[0]:
            ...
            print(
                "HEC-RAS computed unsuccessfully. The HEC-RAS Controller's Compute_CurrentPlan() function returned "
                "False."
            )
            bRunSucessful = False
    ...
    def get_plan_names(self, IncludeOnlyPlansInBaseDirectory):
        ...
        if self._RASController is None:
            print(
                "RASController has not been created yet. Call init_model() first to create a RASController."
            )

        # it returns PlanCount, PlanNames and IncludeOnlyPlansInBaseDirectory (temp; not used)
        PlanCount, PlanNames, temp = self._RASController.Plan_Names(
            None, None, IncludeOnlyPlansInBaseDirectory
        )
