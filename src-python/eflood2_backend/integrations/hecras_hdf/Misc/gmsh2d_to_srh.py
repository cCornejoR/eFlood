...
def gmsh2d_to_srh(
    ...
    shift_y=0.0,
    units="Meters",
    bAddMonitoringLines=False,
    monitoringLines=[],
    monitoringlineTol=0.001,
):
    ...
    nodeStrings = build_nodeStrings(
        mesh, bAddMonitoringLines, monitoringLines, monitoringlineTol
    )
...
def build_nodeStrings(mesh, bAddMonitoringLines, monitoringLines, monitoringlineTol):
    ...
    for nodeStringID in unique_line_physical_group_IDs:
        ...
        for line_group_ID, line_ID in zip(
            lines_physical_group_IDs, range(len(lines_physical_group_IDs))
        ):
            ...
            if nodeStringID == line_group_ID:
                ...
                if (lines_nodes[line_ID][0] + 1) not in current_node_list:
                    current_node_list.append(
                        lines_nodes[line_ID][0] + 1
                    )  # +1 because meshio is 0-based; gmsh is 1-based
                ...
                if (
                    lines_nodes[line_ID][0] + 1
                ) in current_node_list and current_node_list[-1] != (
                    lines_nodes[line_ID][0] + 1
                ):
                    current_node_list.append(
                        lines_nodes[line_ID][0] + 1
    ...
    if bAddMonitoringLines:
        ...
        for monitoringLine in monitoringLines:
            ...
            for edge in internal_edges:
                ...
                distanceNodeStart = point_to_segment_distance(
                    coordNodeStart[0],
                    coordNodeStart[1],
                    xML_start,
                    yML_start,
                ...
                distanceNodeEnd = point_to_segment_distance(
                    coordNodeEnd[0],
                    coordNodeEnd[1],
                    xML_start,
                    yML_start,
                ...
                if (
                    distanceNodeStart < monitoringlineTol
                    and distanceNodeEnd < monitoringlineTol
                ):
...
def write_srhgeom(
    srhmatFileName, mesh, nodeStrings, shift_x=0.0, shift_y=0.0, units="Meters"
):
    """
    ...
    for pointI in range(mesh.points.shape[0]):
        ...
        curr_point_coordinates = [
            mesh.points[pointI, 0] + shift_x,
...
def write_srhmat(
    srhmatFileName, nManningNZones, ManningNZoneNames, cellsInManningZones
):
    ...
    fid.write(
        "NMaterials %d\n" % (nManningNZones + 1)
    )  # +1 is because SRH-2D also counts the default Manning's n in srhhydro file.

    # output MatName
    for matID in range(nManningNZones):
        fid.write(
            'MatName %d "%s" \n' % (matID + 1, ManningNZoneNames[matID])
    ...
    for matID in range(nManningNZones):
        ...
        for cellI in range(len(cellsInManningZones[matID])):
            ...
            if (((cellI + 1) % 10) == 0) or (
                cellI == (len(cellsInManningZones[matID]) - 1)
            ):
                fid.write("\n")
