...
class vtkHandler:
    ...
    def read_sampling_points(
        self, num_sampling_files, sampling_file_names, output_vtp_file_name
    ):
    ...
    def probeUnstructuredGridVTKOnPoints(
        self,
        pointVTK,
        ...
        return self.probeUnstructuredGridVTKOverLine(
            pointVTK, readerUnstructuredGridVTK, varName, kernel, radius, nullValue
        )

    def probeUnstructuredGridVTKOverLine(
        self,
        lineVTK,
        readerUnstructuredGridVTK,
        varName,
        ...
        cell2point.Update()
        data = (
            cell2point.GetOutput()
        )  # after this, all data are stored at points, not cell centers.
        ...
        if radius is None:
            boundingArea = (bounds[1] - bounds[0]) * (
                bounds[3] - bounds[2]
            )  # assume 2D Grid
            averageCellArea = (
                boundingArea / data.GetNumberOfCells()
            )  # average cell area
        ...
        if isinstance(lineVTK, vtk.vtkLineSource):
            ...
            raise Exception(
                "lineVTK type,",
                type(lineVTK),
                ", not supported. Only vtkLineSource and vtkPoints are supported.",
            )
        ...
        varProbedValues = VN.vtk_to_numpy(
            probe.GetOutput().GetPointData().GetArray(varName)
        )

        numPoints = (
            probe.GetOutput().GetNumberOfPoints()
    ...
    def vtk_diff_consistent(
        ...
        if (data1.GetNumberOfCells() != data2.GetNumberOfCells()) or (
            ...
            print(
                "Vtk files 1 and 2 are not consistent. They have different number of cells and/or points. Exiting ..."
            )
            sys.exit()
        ...
        if diffNodal:  # if the intended diff is nodal
            if (
                not varName1 in pointDataNames1
            ):  # varName1 does not exist in VTK file1's point data:
                if (
                    varName1 in cellDataNames1
                ):  # varName1 exists in VTK file1's cell data; interpolate that to node
                    print(
                        "varName1 = ",
                        varName1,
                        "does not exist in the point data, but does in cell data. "
            ...
            if (
                not varName1 in cellDataNames1
            ):  # varName1 does not exist in VTK file1's cell data:
                if (
                    varName1 in pointDataNames1
                ):  # varName1 exists in VTK file1's point data; interpolate that to cell
                    print(
                        "varName1 = ",
                        varName1,
                        "does not exist in the cell data, but does in point data. "
        ...
        print(
            "pointDataNames, cellDataNames in second VTK file =",
            pointDataNames2,
            cellDataNames2,
        )
        ...
        if diffNodal:  # if the intended diff is to be at nodal
            if (
                not varName2 in pointDataNames2
            ):  # varName2 does not exist in VTK file2's point data:
                if (
                    varName2 in pointDataNames2
                ):  # varName2 exists in VTK file2's cell data; interpolate that to node
                    print(
                        "varName2 = ",
                        varName2,
                        "does not exist in the point data, but does in cell data. "
            ...
            if (
                not varName2 in cellDataNames2
            ):  # varName2 does not exist in VTK file2's cell data:
                if (
                    varName2 in pointDataNames2
                ):  # varName2 exists in VTK file2's point data; interpolate that to cell
                    print(
                        "varName2 = ",
                        varName2,
                        "does not exist in the cell data, but does in point data. "
        ...
        varName1_data = (
            VN.vtk_to_numpy(data1.GetPointData().GetArray(varName1))
            if diffNodal
            else VN.vtk_to_numpy(data1.GetCellData().GetArray(varName1))
        )
        varName2_data = (
            VN.vtk_to_numpy(data2.GetPointData().GetArray(varName2))
            if diffNodal
    ...
    def get_uGRid_cell_field_with_name(self, readerUnstructuredGridVTK, varName):
        ...
        if varName not in cellDataNames:
            print(
                "The specified varName:",
                varName,
                "is not in the cell data names: ",
                cellDataNames,
            )
    ...
    def vtk_cell_to_point_interpolation(self, uGrid, varName=""):
        ...
        if varName in pointDataNames:
            print(
                "varName = ",
                varName,
                "is already in the point data. Nothing to be done.",
            )
            return
