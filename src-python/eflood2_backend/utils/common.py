...
def validate_file_path(file_path: Union[str, Path], extensions: Optional[List[str]] = None) -> Path:
    """
    ...
    if extensions and path.suffix.lower() not in [ext.lower() for ext in extensions]:
        raise ValueError(f"Invalid file extension. Expected {extensions}, got: {path.suffix}")
