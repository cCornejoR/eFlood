...
export const ProfessionalManningTable: React.FC<ProfessionalManningTableProps> = ({
  state,
}) => {
  ...
  const hasManningData = manningData?.success &&
  ...
  const exportToCSV = () => {
    const headers = ['ID', 'Tipo de Cobertura', 'Manning n', 'Categoría', 'Descripción'];
    const csvContent = [
      headers.join(','),
      ...tableData.map(row => [
        row.id,
          ...
          {manningData && !manningData.success && (
            <div className="bg-red-500/10 border border-red-500/20 rounded-lg p-3 mt-4">
              <p className="text-red-400 text-xs mb-2">
                Error: {manningData.error || 'Error desconocido al extraer valores de Manning'}
              </p>
              <p className="text-red-300 text-xs">
                💡 Tip: Asegúrate de que el archivo LandCover.hdf esté en la misma carpeta que tu archivo HDF principal.
              </p>
            </div>
          )}
          {!manningData && (
            <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 mt-4">
              <p className="text-blue-400 text-xs mb-1">
                Los datos de Manning se extraerán automáticamente durante el análisis.
              </p>
              <p className="text-blue-300 text-xs">
                🔍 Se buscará en el archivo HDF principal y en archivos LandCover.hdf asociados.
              </p>
            </div>
          )}
        </div>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <TreePine className="h-6 w-6 text-green-400" />
          <div>
            <h3 className="text-lg font-semibold text-white">Valores de Manning Calibrados</h3>
      ...
      {stats && showDetails && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white/5 rounded-lg p-3 border border-white/10">
            ...
            <p className="text-lg font-bold text-yellow-400">{stats.avg.toFixed(3)}</p>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 pt-4 border-t border-gray-800 flex items-center justify-between text-sm text-gray-400">
        ...
        {stats && (
          <div className="flex items-center gap-2">
            ...
            <span>Rango: {stats.min.toFixed(3)} - {stats.max.toFixed(3)}</span>
          </div>
        )}
      </div>
    </motion.div>
...
function getManningCategory(value: number): string {
  if (value < 0.020) return "Muy Lisa";
  if (value < 0.030) return "Lisa";
  if (value < 0.040) return "Moderada";
  if (value < 0.050) return "Rugosa";
  if (value < 0.070) return "Muy Rugosa";
  if (value < 0.100) return "Extrema";
  return "Alta Resistencia";
}

function getManningColor(value: number): string {
  if (value < 0.020) return "bg-blue-500 text-blue-400";
  if (value < 0.030) return "bg-cyan-500 text-cyan-400";
  if (value < 0.040) return "bg-green-500 text-green-400";
  if (value < 0.050) return "bg-yellow-500 text-yellow-400";
  if (value < 0.070) return "bg-orange-500 text-orange-400";
  if (value < 0.100) return "bg-red-500 text-red-400";
